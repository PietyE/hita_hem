import {
  ...
  SET_SELECTED_TAB, SET_INVEST_COMPANIES_LIST,
  SET_FILTER, SET_POSTS,RESET_COMPANY_TAB,
  RESET_COMPANY_LIST,
  IS_MORE_COMPANIES,
} from 'constants/actionsConstant'

import { companyTabConstants } from 'constants/companyTabConstant'
import {chooseCorrectResolution, convertStatusToText} from '../../utils/utils';

const setFilter =(state, actions) => {
  if(Array.isArray(actions.payload)){
    return { ...state, filter: actions.payload }
  }else{
    const isContains = state.filter.find(filter => filter === actions.payload)
    if(!isContains){
...
export const getCompanyIdSelector = (state) =>
    state.companies.companyDetail.id
...
export const getIsFetchingCampaignsSelector = state => state.companies.isFetching
...
export const getFaqAllPostsSelector = (state) =>
    state.companies.faq_posts

export const getFaqUsersPostsSelector = (state) => state.companies.faq_posts.filter(el => el.question === null)
...
export const companies = (state = initialState, actions) => {
  switch (actions.type) {
    ...
    case SET_COMPANIES_LIST:
      return { ...state, companiesList: [...state.companiesList, ...actions.payload] }
