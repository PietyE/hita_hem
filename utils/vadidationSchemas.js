...
//eslint-disable-next-line
const personalIdRegExp =  /^([a-zA-Z\d]{1})([+/a-z A-Z\d-\.,]{6,11})([a-zA-z1\d]{1})$/
...
export const personalDetailsUpdateSchema = yup.object({
    address: yup.object().shape({
        country: yup.string(),
        city: yup.string().test('city', '"City" cannot be empty', val => val?.length),
        address: yup.string().test('address', '"Address" cannot be empty', val => val?.length),
    }),
        first_name: yup.string().test('first_name', '"First name" cannot be empty', val => val?.length),
        second_name: yup.string().test('second_name', '"Last name" cannot be empty', val => val?.length),
...
export const accountSettingsResetEmailSchema = yup.object({
    email: yup.string().email('example test@test.com').required('Email is a required field'),
    password: yup
        .string()
...
export const accountSettingsResetPasswordSchema = yup.object({
    old_password: yup
        .string()
        .required('Password is a required field'),
    new_password1: yup.string().matches(passwordRegExp, 'Please choose a stronger password.').required('New password is a required field'),
    new_password2: yup
        .string().required('Confirm password is a required field')
        .when('new_password1', {
...
export const raiseForm1 = yup.object({
    first_name: yup.string().min(2).max(15).required('First name is a required field'),
    second_name: yup.string().min(2).max(15).required('Last name is a required field'),
    email: yup.string().email('example test@test.com').required('Email is a required field'),
...
export const raiseForm2 = yup.object({
    company_name: yup.string().min(2).max(50).required('Company name is a required field'),
    company_form: yup.string().required('Form of the company is a required field'),
