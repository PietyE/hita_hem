    ...
    useEffect(() => {
        if (matchesAll) {
            let timerId = setInterval(() => {
                if (contentRef?.current?.offsetHeight !== 0 && contentRef?.current?.offsetHeight !== 518) {
    ...
    return (
        <section className = "fin_article_wrapper">
            <li className = "fin_article">
              ...
              <span className = "show_more_button fin_show_more" onClick = {_handleClickShowMore}>
                {isShowMore ? 'Show less' : 'Show more'}
              </span>
                    </div>
                )}
                {!matchesAll && !isShowButton && image && (
                    <Button onClick = {_handleClickShowMore} colorStyle = 'red-without-border'
